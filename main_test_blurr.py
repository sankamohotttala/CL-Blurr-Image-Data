...
#but specifically for testing the model with blurring images
import torch
...
class Blurr_images(torch.nn.Module):# this class is only for testing
    def __init__(self,keranl_size=3, iterations=1):
        super(Blurr_images, self).__init__()
        self.name = 'Blurr_images'
        self.kernal_size = keranl_size
    ...
    def forward(self, img,*args, **kwargs):  
        ...
        cv2_image = image_array.copy() #do not change this variable
        imag_blur = image_array.copy()
        ...
        for _ in range(iterations):
            imag_blur = cv.blur(imag_blur,(kernal_size,kernal_size))
            # imag_blur = cv.GaussianBlur(imag_blur,(kernal_size,kernal_size),0)
        # visualize_images_(cv2_image,imag_blur)
        return imag_blur
         

def visualize_images_(image_orig, image_transf):
    im_list = [image_orig, image_transf]
    plt.figure(figsize=(10, 2))  # Set the figure size
    for i in range(2):
        plt.subplot(1, 2, i+1)  # Create subplots
        plt.imshow(im_list[i])  # Display the image
...
transform_test = transforms.Compose([
    Blurr_images(keranl_size=3, iterations=50),
    transforms.ToTensor(),
